...
// MODIFICATO: Aggiunti Row e Col all'importazione
...
const ConfirmationStep = ({ orderNumber, shippingData, paymentData, outfit }) => {
    const subtotal = outfit.totalPrice;
    const shippingCost = outfit.shippingCost || 0;
    const discountAmount = outfit.discountAmount || 0;
    ...
    return (
        <div className="text-center">
            ...
            <Alert variant="light-checkout-info" className="text-start mt-4 mb-4">
                ...
                <p className="mb-0">Riceverai un'email di conferma all'indirizzo <strong>{shippingData.email}</strong> con tutti i dettagli.</p>
            </Alert>
            ...
            <Card className="text-start order-details-card">
                ...
                <Card.Body>
                    ...
                    <p className="mb-1"><strong>{outfit.name}</strong></p>
                    ...
                    <ListGroup variant="flush" className="summary-details-list mb-2">
                        ...
                        {discountAmount > 0 && (
                             <ListGroup.Item className="d-flex justify-content-between text-success">
                                ...
                                <span>-{outfit.currency}{discountAmount.toFixed(2)}</span>
                            </ListGroup.Item>
                        )}
                    </ListGroup>
                    <p className="order-details-total mt-2"><strong>Totale Pagato:</strong> {outfit.currency}{totalPaid.toFixed(2)}</p>


                    <hr className="my-3"/>

                    <Row> {/* Questa Row era il problema */}
                        <Col md={6}> {/* Questo Col era il problema */}
